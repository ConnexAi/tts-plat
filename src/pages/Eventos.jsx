...
function TarjetaEvento({ evento, cambiandoEstado, onEditar, onToggleEstado, onEliminar }) {
  ...
  // Formatear fecha — rango si hay fecha_fin, fecha simple si no
  const fechaFormateada = evento.fecha_fin
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5 flex flex-col gap-4 hover:shadow-sm transition-shadow">
      ...
      <div className="pt-1 border-t border-gray-50 space-y-2">
        ...
        {/* Fila 1 — módulos de navegación (cuadrícula 2×2) */}
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={() => navegar(`/inventario/${evento.id}`)}
            className="flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
          >
          ...
          <button
            onClick={() => navegar(`/asistencia/${evento.id}`)}
            className="flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            onClick={() => navegar(`/evidencias/${evento.id}`)}
            className="flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            onClick={() => navegar(`/informe/${evento.id}`)}
            className="flex items-center justify-center gap-1.5 py-2 text-xs font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors"
          >
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Fila 2 — gestión del evento */}
        <div className="flex gap-2">
          {/* Cambiar estado */}
          <button
            onClick={onToggleEstado}
            disabled={cambiandoEstado}
            className="flex-1 py-1.5 text-xs font-medium text-gray-500 hover:text-gray-800 hover:bg-gray-50 rounded-lg transition-colors disabled:opacity-40"
          >
            {cambiandoEstado ? '…' : esActivo ? 'Cerrar evento' : 'Reactivar'}
          </button>
          ...
          <button
            onClick={onEditar}
            className="px-3 py-1.5 text-xs font-medium text-gray-500 hover:text-gray-800 hover:bg-gray-50 rounded-lg transition-colors"
          >
            Editar
          </button>
          ...
          <button
            ...
            className="px-2.5 py-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"
